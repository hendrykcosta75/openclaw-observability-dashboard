...
export function LogsPage() {
  const { logRows, logWindowBuckets, snapshotMeta, stateCards } = useSnapshot();
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="space-y-5">
        <SectionTitle eyebrow="Janelas" title="Contadores de erro por período" description="Contadores agregados por linha de log; a janela de 7 dias usa datas reconhecidas nos arquivos." icon={ScrollText} />
      ...
      <section className="space-y-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {stateCards.map((card) => {
            const Icon = getSnapshotIcon(card.icon);
            return (
      ...
      <section className="space-y-5">
        ...
        <Card className="overflow-hidden p-0">
          <div className="grid grid-cols-[1.4fr_.7fr_.45fr_.45fr_.45fr_.55fr] gap-3 border-b border-border-dim bg-dim px-4 py-3 text-[11px] uppercase tracking-[0.14em] text-subtle" style={mono}>
            <span>Fonte</span><span>Amostra</span><span>Erros</span><span>Deferred</span><span>Sucesso</span><span>% marcador</span>
          </div>
          {logRows.map((row, index) => {
            const successRate = row.sampledLines > 0
              ? `${Math.round(((row.success ?? 0) / row.sampledLines) * 100)}%`
              : "—";
            return (
