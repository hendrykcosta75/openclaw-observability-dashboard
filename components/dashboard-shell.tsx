      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4" aria-label="Dashboard navigation">
        ...
        <ul className="space-y-1">
          {sidebarNav.map((item, index) => {
            ...
            return (
              <li key={item.label}>
                <a href={item.href} className={`flex items-center gap-2.5 px-3 py-2 text-[12px] transition-all ${index === 0 ? "sidebar-item-active" : "rounded-[10px] text-[rgba(255,255,255,0.45)] hover:bg-[rgba(255,255,255,0.03)] hover:text-[rgba(255,255,255,0.88)]"}`} style={mono}>
...
function Header() {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <span className="text-gradient text-[13px] font-bold uppercase tracking-[2px]" style={mono}>OpenClaw Dashboard</span>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button size="sm" variant="ghost" className="h-8 min-w-0 rounded-[9px] border-none bg-dim px-3 text-xs text-subtle hover:text-heading" onPress={handleLogout}>
          <LogOut size={13} />
          Sair
        </Button>
      </div>
    ...
    <Card id="custos" className="p-4 lg:col-span-5">
      <Card.Header className="flex flex-row items-start justify-between p-0">
        <div>
          ...
          <Card.Description className="text-xs text-subtle">Fontes de custo mapeadas; valores reais entram após instrumentação.</Card.Description>
        </div>
...
function PlanPanel() {
  return (
    <section id="plano" className="space-y-5">
      <SectionTitle eyebrow="Harness" title="Plano de evolução" description="Sequência de trabalho para transformar o snapshot em observabilidade viva." icon={ListChecks} />
        ...
        <main className="relative z-10 flex-1 overflow-y-auto p-5 lg:p-6">
          <div className="mx-auto max-w-7xl space-y-10">
              ...
              <div className="grid gap-5 lg:grid-cols-[1.3fr_.7fr] lg:items-end">
                <div>
                  <h1 className="max-w-4xl text-3xl font-semibold leading-tight text-heading md:text-5xl" style={mono}>
                    OpenClaw <span className="text-gradient">Operations</span>
                  </h1>
                  <p className="mt-4 max-w-3xl text-sm leading-relaxed text-subtle md:text-base">
                    Leitura operacional do gateway, agentes, crons, custos e logs. Os dados desta fase vêm do inventário agregado do VPS e servem como base para a coleta viva.
                  </p>
                </div>
                <Card className="p-4">
                  <Card.Header className="flex flex-row items-center justify-between p-0">
                    <div>
                      <Card.Title className="text-base text-heading" style={mono}>Snapshot</Card.Title>
                      <Card.Description className="text-xs text-subtle">Coleta agregada do VPS OpenClaw.</Card.Description>
                    </div>
            ...
            <section id="design-system" className="space-y-5">
              <SectionTitle eyebrow="Design system" title="Baisync-like UI harness" description="Componentes, tokens e regras visuais para manter a interface consistente." icon={Sparkles} />
              <Card className="p-4">
                <Card.Content className="grid gap-4 p-0 md:grid-cols-3">
                  ...
                  <div className="rounded-[14px] border border-border-dim bg-dim/50 p-3">
                    ...
                    <p className="mt-2 text-xs text-subtle">AGENTS.md e skills locais mantêm UI e coleta em um fluxo verificável.</p>
                  </div>
                </Card.Content>
              </Card>
            </section>
              ...
              <Card className="p-4">
                <Card.Content className="flex flex-col gap-4 p-0 md:flex-row md:items-center md:justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-base text-heading" style={mono}>Pipeline de coleta</p>
                      <p className="mt-1 max-w-3xl text-xs leading-relaxed text-subtle">
                        A próxima iteração deve gerar JSON por timer, alimentar o Next.js e registrar histórico diário de custo, jobs e incidentes.
                      </p>
                    </div>
                  </div>
